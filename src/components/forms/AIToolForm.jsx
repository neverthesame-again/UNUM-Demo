// AIToolForm Component - Add/Edit AI tool form
...
export const AIToolForm = ({
  ...
  // Populate form if editing existing tool
  useEffect(() => {
    if (tool) {
      setFormData({
        ...
        display_order: tool.display_order || 999,
      });
    }
  }, [tool]);

  // Surface duplicate-title errors from the backend (e.g. a race with
  // another admin adding the same AI tool between load and submit).
  useEffect(() => {
  ...
  const validate = () => {
    ...
    if (!formData.title.trim()) {
      newErrors.title = "AI tool title is required";
    } else if (
      formData.domain_id &&
      aiTools.some(
        (t) =>
      ...
      newErrors.title = "An AI tool with this title already exists in this domain";
    }
    ...
    <form className="domain-form" onSubmit={handleSubmit}>
      ...
      <div className="form-group">
        ...
        <input
          ...
          onChange={handleChange}
          placeholder="e.g., Copilot IDE Plugin"
        />
      ...
      <div className="form-actions">
        ...
        <button type="submit" className="btn-primary">
          {tool ? "Update AI Tool" : "Add AI Tool"}
        </button>
      </div>
    </form>
  );
